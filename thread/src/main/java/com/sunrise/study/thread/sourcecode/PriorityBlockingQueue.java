...
public class PriorityBlockingQueue<E> extends AbstractQueue<E>
     ...
     * 存放队列元素。
     * 优先队列，是由平衡二叉堆实现：
     * queue[n]的两个孩子节点是queue[2*n+1]和queue[2*(n+1)]。
     * 优先队列是通过用户自定义的比较器comparator或元素自身的顺序排列。
    ...
    /**
     * 用于扩容的自旋锁。状态为0或者1，0表示没有进行扩容，1表示正在扩容。
    ...
    private void tryGrow(Object[] array, int oldCap) {
        ...
        // 通过CAS将allocationSpinLock由0设为1，即获取自旋锁
        if (allocationSpinLock == 0 &&
            UNSAFE.compareAndSwapInt(this, allocationSpinLockOffset,
                                     0, 1)) {
            try {
                // 计算新容量，若oldCap小于64，就是 oldCap+oldCap+2，否则就是 oldCap+oldCap>>1。
        ...
        // 若有其他线程在扩容，CAS执行成功后，当前线程会进入这段代码，让出CPU给前面的线程获取锁。
        if (newArray == null) // back off if another thread is allocating
            Thread.yield();
        // 获取独占锁，将newArray赋值给queue字段，同时通过本地方法复制元素到新数组
        lock.lock();
    ...
    /**
     * 在位置k处插入元素x，然后对其进行下沉操作（使用元素自身的compareTo()方法比较）
     * Inserts item x at position k, maintaining heap invariant by
    ...
    /**
     * 同siftDownComparable()方法（使用自定义比较器）
     */
