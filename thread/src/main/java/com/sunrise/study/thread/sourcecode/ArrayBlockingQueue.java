...
public class ArrayBlockingQueue<E> extends AbstractQueue<E>
    ...
    private void enqueue(E x) {
        ...
        items[putIndex] = x;                // 在putIndex位置插入值
        if (++putIndex == items.length)     // putIndex索引递增，若等于队列长度，则从0开始
            putIndex = 0;
        count++;            // 队列元素个数+1
    ...
    private E dequeue() {
        ...
        final Object[] items = this.items;
        @SuppressWarnings("unchecked")
        E x = (E) items[takeIndex];     // 获取takeIndex索引处的元素值
        items[takeIndex] = null;        // 获取后设为null，表示从队列中移除
        if (++takeIndex == items.length)    // 获取后，takeIndex+1，若等于队列长度，则从0开始
            takeIndex = 0;
        count--;            // 队列元素个数-1
        if (itrs != null)   // 若itrs不为null，则调用itrs.elementDequeued()方法
    ...
    void removeAt(final int removeIndex) {
        ...
        final Object[] items = this.items;
        // 如果removeIndex == takeIndex，就是出队操作，代码和dequeue()类似
        if (removeIndex == takeIndex) {
            ...
            if (itrs != null)
                itrs.elementDequeued();
        } else {    // 如果不是出队操作，就从removeIndex索引开始，将后面的元素都往前移动一个位置
            // an "interior" remove
    ...
    /**
     * 入队。当队列满时，当前线程会进入notFull条件队列等待指定时间。
     * Inserts the specified element at the tail of this queue, waiting
    ...
    /**
     * 出队。若队列为空，当前线程会进入notEmpty条件队列排队，直到获取到元素。会抛出中断异常。
     */
    ...
    /**
     * 出队。若队列为空，当前线程会进入notEmpty条件队列等待指定时间，出队失败返回null。会抛出中断异常。
     */
