...
/**
 * 总结：
 * 1、WeakHashMap的Entry类继承了WeakReference类，使用Reference类的referent字段作为key；
 * 2、由于WeakHashMap中的entry的key是弱引用，所以entry随时会被GC回收；
 * 3、
 * @author huangzihua
 * @date 2021-08-27
 */
public class WeakHashMap<K,V> extends AbstractMap<K,V> implements Map<K,V> {
    ...
    /**
     * null值转换，把null转换为NULL_KEY对象，使用NULL_KEY代替null值
     */
    ...
    /**
     * 获取key对应的value值，若找不到则返回null
     * 底层是通过while循环比对table中的entry.key是否相等，相等则返回entry.value
    ...
    void resize(int newCapacity) {
        ...
        // 迁移所有元素到新table中（重新进行了一遍散列操作）
        transfer(oldTable, newTable);
        table = newTable;

        /*
         * 前面transfer()方法会清除部分元素，如果清除后table的元
         * 素大幅度减少了，则恢复旧table。不过这种情况很少发生。
    ...
    /**
     * 将Entry数组src中的所有元素移到dest数组中。只有resize()方法中调用，用于扩容后把元素迁移到新的table。
     * 底层实现是：遍历整个src数组，参照扩容后的长度，重新计算
     * 一遍每个元素的hash值，添加到新数组对应的索引中。
    ...
    private void transfer(Entry<K,V>[] src, Entry<K,V>[] dest) {
        for (int j = 0; j < src.length; ++j) {
            ...
            while (e != null) {
                ...
                Object key = e.get();   // 调用Reference的get()方法，获取key
                // 若key为null，说明被GC回收了，则删除该节点
                if (key == null) {
                    e.next = null;  // Help GC
                    e.value = null; //  "   "
                    size--;
                } else {
                    int i = indexFor(e.hash, dest.length);  // 这里重新计算了索引值
                    e.next = dest[i];
                    dest[i] = e;
                }
                e = next;
    ...
    /**
     * 该Entry继承了WeakReference类，使用该类的referent字段存放key，实现弱引用的功能
     */
